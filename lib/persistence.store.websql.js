...
persistence.store.websql.config = function(persistence, dbname, description, size) {
  ...
  /**
    * Add alter column capability for html5 websql,  please  config the persistence.version and persistence.alterColumnQueries in your code,
    * For example:
    * persistence.version = 2.0;
    * persistence.queries = ['ALTER TABLE User ADD COLUMN address TEXT'];
    * If you have column change in your code, just change the persistence.version = 2.0 ,and add your alter sql in persistence.queries , while you can not delete the column. Good luck. 
    */
  persistence.db.html5.connect = function (dbname, description, size) {
    var that = {},
          q
          ;
    var conn = openDatabase(dbname, '', description, size);

    if (persistence.debug) {
        console.log('Current Version:' + conn.version);
        console.log('Latest Version:' + conn.version);
    }

    if (conn.version + '' !== persistence.version + '' && conn.version){
        
        conn.changeVersion(conn.version, persistence.version, function(t) {
                for (q in persistence.alterColumnQueries) {
                        
                        if (persistence.debug) {
                                console.log('Execute alter column SQL: ' + persistence.alterColumnQueries[q]);
                        }
                        t.executeSql(persistence.alterColumnQueries[q]);
                }
        });
    }
  ...
  persistence.db.html5Sync.connect = function (dbname, description, size) {
    var that = {};
    var conn = openDatabaseSync(dbname, '', description, size);
